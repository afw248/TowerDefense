...
public class PlayerIdleState : AbstractPlayerState
{
    ...
    private const float ATTACK_DELAY = 0.05f;
    ...
    private void CheckAttack()
    {
        ...
        if (_player.Sensor.FindTargetsInRadius(_player.PlayerData.DetectRadius) > 0)
        {
            _attackReadyTimer = 0f;

            _player.ChangeState(PlayerState.FIRE, 0.05f);
        }
    }
}